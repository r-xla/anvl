...
prim_log[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  grad <- grads[[1L]]
  list(
    if (required[[1L]]) prim_div(grad, operand)
  )
})
...
prim_abs[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(
    # d/dx |x| = sign(x)
    if (required[[1L]]) prim_mul(grad, prim_sign(operand))
  )
})
...
prim_dot_general[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  # remaining axes
  rem_axes <- function(operand, b_axes, c_axes) {
    ii <- c(b_axes, c_axes)
    seq_len(naxes(operand))[if (length(ii)) -ii else TRUE]
  }
...
prim_reshape[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  grad <- grads[[1L]]
  list(
    if (required[[1L]]) prim_reshape(grad, shape(operand))
  )
})
...
prim_reduce_sum[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(
    if (required[[1L]]) {
      baxes <- if (drop) {
        without(seq_along(shape(operand)), axes)
      } else {
...
prim_broadcast_in_axes[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(
    if (required[[1L]]) {
      ...
      expand_axes <- broadcast_axes[(shape(y)[broadcast_axes] != 1L) & (shape(operand) == 1L)]
      reduce_axes <- c(new_axes, expand_axes)

      x <- if (length(reduce_axes)) prim_reduce_sum(grad, axes = reduce_axes, drop = FALSE) else grad

      # Drop the singular added axes
      if (length(new_axes)) {
        reshape_dims <- shape(x)
        reshape_dims <- reshape_dims[-new_axes]
        x <- prim_reshape(x, reshape_dims)
      }
      ...
      if (is.unsorted(broadcast_axes)) {
        x <- prim_transpose(x, order(broadcast_axes))
      }
      x
    }
  )
...
prim_convert[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(
    if (required[[1L]]) prim_convert(grad, dtype(operand), inputs[[1L]]$gnode$aval$ambiguous)
  )
})
...
prim_expm1[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(
    # d/dx (exp(x) - 1) = exp(x)
    if (required[[1L]]) prim_mul(grad, prim_exp(operand))
  )
})
...
prim_log1p[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(
    ...
    if (required[[1L]]) {
      one <- prim_fill(1, dtype = dtype(operand), shape = shape(operand))
      prim_div(grad, prim_add(one, operand))
    }
  )
})
...
prim_clamp[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  mask_operand <- prim_convert(prim_eq(operand, y), dtype = dtype(grad))

  list(
    if (required[[1L]]) cli_abort("Gradient for min_val not implemented"),
    if (required[[2L]]) prim_mul(grad, mask_operand),
...
prim_reduce_all[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  operand <- inputs[[1L]]
  list(
    if (required[[1L]]) prim_fill(FALSE, dtype = dtype(operand), shape = shape(operand))
  )
})
...
prim_top_k[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  list(prim_scatter(
    input = zero_input,
    # index vectors selecting positions along the last axis of `input`
    scatter_indices = indices,
    update = grad_values,
    update_window_axes = integer(0),
    inserted_window_axes = rank,
    input_batching_axes = batching,
    scatter_indices_batching_axes = batching,
    scatter_axes_to_operand_axes = rank,
...
.cum_extreme_reverse <- function(inputs, outputs, grads, params, required) {
  ...
  list(prim_scatter(
    ...
    input_batching_axes = batching,
    scatter_indices_batching_axes = batching,
    scatter_axes_to_operand_axes = axis,
    index_vector_axis = rank + 1L,
...
prim_gather[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  operand_batching_axes <- params$operand_batching_axes
  ...
  list(
    if (required[[1L]]) {
      ...
      prim_scatter(
        ...
        input_batching_axes = operand_batching_axes,
        scatter_indices_batching_axes = start_indices_batching_axes,
        scatter_axes_to_operand_axes = start_index_map,
        index_vector_axis = index_vector_axis,
...
prim_scatter[["reverse"]] <- rule_reverse(function(inputs, outputs, grads, params, required) {
  ...
  input_batching_axes <- params$input_batching_axes
  scatter_indices_batching_axes <- params$scatter_indices_batching_axes
  scatter_axes_to_operand_axes <- params$scatter_axes_to_operand_axes
  index_vector_axis <- params$index_vector_axis
  indices_are_sorted <- params$indices_are_sorted
  unique_indices <- params$unique_indices
    ...
    if (required[[3L]]) {
      if (unique_indices) {
        prim_gather(
          ...
          operand_batching_axes = input_batching_axes,
          start_indices_batching_axes = scatter_indices_batching_axes,
          start_index_map = scatter_axes_to_operand_axes,
          index_vector_axis = index_vector_axis,
          indices_are_sorted = indices_are_sorted,
          unique_indices = TRUE
        )
