...
trace_fn <- function(
  ...
  # its R values materialize when `maybe_box_input()` builds the parameter slots.
  # We might
  if (mode == "toplevel") {
    # Standard case:
...
graph_desc_add <- function(primitive, args, params = list(), infer_fn, desc = NULL, device = NULL) {
  ...
  for (i in seq_len(n_in)) {
    # Materialize R values at their default dtype, which happens when no promotion rule
