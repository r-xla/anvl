...
prim_convolution <- new_primitive(
  "convolution",
  function(lhs, rhs, dimension_numbers, window_strides, padding,
           lhs_dilation, rhs_dilation, feature_group_count = 1L,
           batch_group_count = 1L, precision = "highest") {
    precision <- match.arg(precision, c("default", "high", "highest"))
    infer_fn <- function(lhs, rhs, dimension_numbers, window_strides, padding,
                         lhs_dilation, rhs_dilation, feature_group_count,
                         batch_group_count, precision) {
      dn <- dimension_numbers
      shlo_dn <- stablehlo::ConvDimensionNumbers(
        input_batch_dimension = dn$input_batch_dimension - 1L,
        input_feature_dimension = dn$input_feature_dimension - 1L,
        input_spatial_dimensions = dn$input_spatial_dimensions - 1L,
        kernel_input_feature_dimension = dn$kernel_input_feature_dimension - 1L,
        kernel_output_feature_dimension = dn$kernel_output_feature_dimension - 1L,
        kernel_spatial_dimensions = dn$kernel_spatial_dimensions - 1L,
        output_batch_dimension = dn$output_batch_dimension - 1L,
        output_feature_dimension = dn$output_feature_dimension - 1L,
        output_spatial_dimensions = dn$output_spatial_dimensions - 1L
      )
      n <- length(dn$input_spatial_dimensions)
      pad <- padding
      ...
      out <- stablehlo::infer_types_convolution(
        ...
        window_strides = r_to_constant(as.integer(window_strides),
          dtype = "i64", shape = length(window_strides)),
        padding = r_to_constant(pad, dtype = "i64", shape = dim(pad)),
        lhs_dilation = r_to_constant(as.integer(lhs_dilation),
          dtype = "i64", shape = length(lhs_dilation)),
        rhs_dilation = r_to_constant(as.integer(rhs_dilation),
          dtype = "i64", shape = length(rhs_dilation)),
        window_reversal = r_to_constant(rep(FALSE, n), dtype = "i1", shape = n),
        feature_group_count = r_to_constant(as.integer(feature_group_count),
          dtype = "i64", shape = integer()),
        batch_group_count = r_to_constant(as.integer(batch_group_count),
          dtype = "i64", shape = integer())
      )[[1L]]
      list(vt2at(out))
    }
