...
prim_reduce <- new_primitive(
  ...
  function(x, init, axes, drop = TRUE, reductor) {
    ...
    # Traced below with exactly two positional arguments. A third one is bound
    # to nothing and the reduction silently returns `init`; stablehlo cannot
    # see this, since by the time it gets the body the arity is already fixed.
