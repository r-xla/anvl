...
#' and emits a registry of functions to wrap in [`jit()`] when the package is
#' loaded.
#'
...
#' 4. In `R/zzz.R`, apply the registry from `.onLoad()`, so the wrappers see
#'    the backends that are active in this session:
#'
#'    ```r
#'    .onLoad <- function(libname, pkgname) {
#'      anvl::apply_jit_registry(.jit_registry, asNamespace(pkgname))
#'    }
#'    ```
#'
...
jit_registry_lines <- function(entries) {
  header <- c(
    ...
    "",
    "# Functions tagged with `#' @jit` are rebound in `.onLoad()` to",
