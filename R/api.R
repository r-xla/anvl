...
#' @return ([`arrayish`])\cr
#'   Has the common data type of `yes` and `no`, and their
#'   broadcast shape.
...
#'   are [promoted to a common data type][nv_promote_to_common()] and that is
#'   then converted to the default float data type (see [`default_dtypes()`])
#'   where it is not a float already.
...
#' @inherit prim_eigen description
...
#'   `indices` when `with_indices = TRUE`. The values have the input's data type
...
#' @export
nv_top_k <- jit(
  function(x, k, axes = NULL, with_indices = FALSE) {
    assert_flag(with_indices)
    x <- as_anvl_array(x)
    rank <- naxes(x)
    if (rank == 0L) {
    ...
    out <- prim_top_k(flat$x, k = k, with_indices = with_indices)
