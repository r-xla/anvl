...
#'   whole number for an unsigned integer and a logical for `bool`. `NA` is
#'   rejected, since XLA has no missing value. Whether the value fits the data
#'   type's range is left to the backend.
...
#'   assumes the data type of the other operand, and commits to its
...
nv_crossprod <- function(lhs, rhs = NULL) {
  ...
  assert_matrix(lhs, "lhs")
  assert_matrix(rhs, "rhs")
...
#' @template param_nan_rm
#' @return ([`arrayish`])\cr
#'   Has a float data type regardless of the input's. For scalar `probs` the
...
#' where the arguments after `x` are matched by name.
#' @templateVar dtypes any data type
#' @template param_unary_x
#' @param axis (`integer(1)` | `NULL`)\cr
...
#' @return ([`arrayish`])\cr
#'   Has a float data type regardless of the input's, and the input's shape with
#'   `axis` removed.
