...
operators). Masks come in two forms: per-axis masks that shrink one axis, and
...
| Form | Example | Result |
|------|---------|--------|
| Per-axis | `x[arr(TRUE, FALSE, TRUE), ]` | Keeps rank; that axis shrinks to `sum(mask)` |
...
size-1 axis. Rather than pick a surprising winner, both are rejected with a hint:
...
> A subscript is a **flat** mask when its shape equals the full shape of `x`.
> Otherwise it is a **per-axis** mask on axis 1, with trailing axes full.
...
The per-axis reading of a single subscript deliberately differs from base R, which
would flatten. It is consistent with anvl's existing convention that `x[2]` on a matrix
means "row 2", not "element 2".

For a rank-1 array the two forms coincide, and both produce the same result.

### Validation

- Length must match exactly: `length(mask) == axis size` per axis, or
...
per-axis mask on the rows of a 3 x 4 array), rather than an error.
...
when there is exactly one subscript and it is a mask whose shape equals the shape of `x`.
...
- `flat_mask_to_gather()`: `slice_sizes = rep(1L, rank)`, `offset_axes = integer()`,
  `collapsed_slice_axes = seq_len(rank)`, `start_index_map = seq_len(rank)`,
  `index_vector_axis = 2L`, `unique_indices = TRUE`.
- `flat_mask_to_scatter()`: `update_window_axes = integer()`,
  `inserted_window_axes = seq_len(rank)`, `scatter_axes_to_x_axes = seq_len(rank)`,
  `index_vector_axis = 2L`, `update_shape = k`, `unique_indices = TRUE`.
...
Zero-sized selections currently fail because a size-0 axis is misclassified as
...
- `dynamic_start_indices()` and `static_start_indices()` -- `multi_index_axes`
...
- Per-axis masks on each axis of a 2-D array, and on a 1-D array.
