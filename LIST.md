...
- Templates: `param_unary_x` for a single arrayish operand, or
  ...
  `nv_*` function that promotes with `promotion_like("x")` instead
...
promote to a common data type through `promotion_common()`, like every other
`nv_*` function; they used to carry the primitive layer's
`promotion_rdata_common()`, which refuses a mixed `f32`/`f64` pair. The solves
...
- `nv_pad()` and `nv_clamp()` refuse an R double for an integer `x`
  (`nv_pad(x_i32, 0)` errors, `0L` works), because `promotion_like("x")` keeps
