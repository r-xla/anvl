...
describe("apply_jit_registry", {
  # A stand-in namespace: functions assigned in definition order, as a package
  # sources them, with the registry applied last (as R/zzz.R does).
  make_namespace <- function() {
    ns <- new.env(parent = topenv())
    ns$f <- function(x, y) x + y
    ns$g <- ns$f
    ns$`floor.Cls` <- ns$f
    ns$h <- function(x) x * 2
    ns$not_a_function <- 42
    ns
  }

  it("rebinds the tagged function to its jitted wrapper", {
    ns <- make_namespace()
    apply_jit_registry(list(list(name = "f", static = character())), envir = ns)
    expect_s3_class(ns$f, "JitFunction")
  })

  it("rebinds aliases of the tagged function to the same wrapper", {
    ns <- make_namespace()
    apply_jit_registry(list(list(name = "f", static = character())), envir = ns)
    expect_identical(ns$g, ns$f)
    expect_identical(ns$`floor.Cls`, ns$f)
  })

  it("leaves untagged functions and non-functions alone", {
    ns <- make_namespace()
    apply_jit_registry(list(list(name = "f", static = character())), envir = ns)
    expect_false(inherits(ns$h, "JitFunction"))
    expect_identical(ns$not_a_function, 42)
  })

  it("passes `static` on to `jit()`", {
    ns <- new.env(parent = topenv())
    ns$k <- function(x, flag) if (flag) x + 1 else x * 2
    ns$k_alias <- ns$k
    apply_jit_registry(list(list(name = "k", static = "flag")), envir = ns)
    expect_equal(ns$k_alias(nv_scalar(3), TRUE), nv_scalar(4))
    expect_equal(ns$k_alias(nv_scalar(3), FALSE), nv_scalar(6))
  })

  it("errors when a binding is an alias of more than one tagged function", {
    ns <- new.env(parent = topenv())
    ns$a <- function(x) x + 1
    ns$b <- ns$a
    ns$both <- ns$a
    expect_error(
      apply_jit_registry(
        list(
          list(name = "a", static = character()),
          list(name = "b", static = character())
        ),
        envir = ns
      ),
      "alias of more than one tagged function"
    )
  })

  it("accepts an empty registry", {
    ns <- make_namespace()
    expect_no_error(apply_jit_registry(list(), envir = ns))
    expect_false(inherits(ns$f, "JitFunction"))
  })
})
