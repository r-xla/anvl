...
test_that("an R value takes the data type of the operand it meets", {
  for (dt in c("f32", "f64")) {
...
test_that("the operand slots that take a scalar accept a literal", {
...
test_that("a literal reaches a primitive with every digit it had", {
...
test_that("a primitive call no longer depends on the default data type", {
...
test_that("an all-R group agrees only within one R storage type", {
...
test_that("a literal is only ever built within its own category", {
...
test_that("a group with several data types present is left to type inference", {
...
test_that("primitives whose operands are meant to differ opt out", {
...
test_that("an argument a rule leaves out keeps its own data type", {
...
test_that("eager and traced primitive calls agree", {
