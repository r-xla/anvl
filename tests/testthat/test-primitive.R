...
test_that("a primitive's arguments are resolved before its body runs", {
  # The rule fires at the entry, not when the call is recorded, so a body that
  # reads a data type or traces a sub-graph first never meets an open R value.
  # prim_reduce() reads dtype(init) twenty lines before it records anything.
  x <- nv_array(c(1, 2, 3), dtype = "f64")
  expect_identical(as_array(prim_reduce(x, 0, axes = 1L, reductor = prim_add)), 6)
  expect_identical(as_array(prim_reduce(x, 1, axes = 1L, reductor = prim_mul)), 6)
  # prim_scatter() builds its update computation's parameter slots from
  # peek_dtype(x), so `update` has to have settled by then too.
  y <- nv_array(c(1, 2, 3), dtype = "f64")
  y[2] <- 0.5
...
test_that("a primitive that promotes nothing leaves its arguments alone", {
  ...
  p <- new_primitive(
    ...
    function(lhs, rhs) {
      ...
      prim_add(lhs, nv_convert(rhs, dtype(lhs)))
    },
    register = FALSE
  )
  invisible(p(nv_scalar(1, dtype = "f64"), 2))
