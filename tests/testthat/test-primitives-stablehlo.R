...
# Convolution: torch-free checks against hand-computed cross-correlation
# (torch parity across shapes/strides/dilation/groups lives in the extra-tests
# block sourced below). NCHW-style layout; nv_array fills column-major, so
# inputs are built as R arrays and passed as `c(arr)` with `shape = dim(arr)`.

nv_arr <- function(a) nv_array(c(a), dtype = "f64", shape = dim(a))

test_that("prim_convolution 1d", {
  x <- array(c(1, 2, 3, 4), dim = c(1, 1, 4))
  w <- array(c(1, 2), dim = c(1, 1, 2)) # cross-correlation kernel [1, 2]

  expect_equal(
    as_array(nv_conv1d(nv_arr(x), nv_arr(w))),
    array(c(5, 8, 11), dim = c(1, 1, 3))
  )
  expect_equal(
    as_array(nv_conv1d(nv_arr(x), nv_arr(w), padding = 1)),
    array(c(2, 5, 8, 11, 4), dim = c(1, 1, 5))
  )
  expect_equal(
    as_array(nv_conv1d(nv_arr(x), nv_arr(w), stride = 2)),
    array(c(5, 11), dim = c(1, 1, 2))
  )

  x5 <- array(c(1, 2, 3, 4, 5), dim = c(1, 1, 5))
  expect_equal(
    as_array(nv_conv1d(nv_arr(x5), nv_arr(w), dilation = 2)),
    array(c(7, 10, 13), dim = c(1, 1, 3))
  )
})

test_that("prim_convolution grouped 1d", {
  # Cin = Cout = 2, groups = 2: each output channel sees one input channel.
  x <- array(0, dim = c(1, 2, 3))
  x[1, 1, ] <- c(1, 2, 3)
  x[1, 2, ] <- c(4, 5, 6)
  w <- array(0, dim = c(2, 1, 2))
  w[1, 1, ] <- c(1, 0) # picks the earlier tap of channel 0
  w[2, 1, ] <- c(0, 1) # picks the later tap of channel 1
  out <- as_array(nv_conv1d(nv_arr(x), nv_arr(w), groups = 2))
  expect_equal(out, array(c(1, 5, 2, 6), dim = c(1, 2, 2)))
})

test_that("prim_convolution 2d", {
  x <- array(0, dim = c(1, 1, 3, 3))
  x[1, 1, , ] <- matrix(1:9, 3, 3, byrow = TRUE)
  w <- array(0, dim = c(1, 1, 2, 2))
  w[1, 1, , ] <- matrix(1:4, 2, 2, byrow = TRUE)
  out <- as_array(nv_conv2d(nv_arr(x), nv_arr(w)))
  expect_equal(out, array(c(37, 67, 47, 77), dim = c(1, 1, 2, 2)))
})

test_that("prim_convolution 3d", {
  x <- array(seq_len(8), dim = c(1, 1, 2, 2, 2))
  w <- array(1, dim = c(1, 1, 2, 2, 2)) # sum over the 2x2x2 window
  out <- as_array(nv_conv3d(nv_arr(x), nv_arr(w)))
  expect_equal(out, array(36, dim = c(1, 1, 1, 1, 1)))
})

test_that("prim_convolution asymmetric (causal) padding", {
  # Left-pad kW - 1, right-pad 0: the causal padding the nv_* wrappers
  # (symmetric only) can't express, exercised through prim_convolution.
  x <- nv_array(c(1, 2, 3, 4), dtype = "f64", shape = c(1, 1, 4))
  w <- nv_array(c(1, 2), dtype = "f64", shape = c(1, 1, 2))
  out <- prim_convolution(
    x,
    w,
  ...
  expect_equal(as_array(out), array(c(2, 5, 8, 11), dim = c(1, 1, 4)))
})

test_that("nv_conv* rejects a stride of the wrong length", {
  x <- nv_array(c(1, 2, 3, 4), dtype = "f64", shape = c(1, 1, 2, 2))
  w <- nv_array(c(1), dtype = "f64", shape = c(1, 1, 1, 1))
  expect_error(nv_conv2d(x, w, stride = c(1, 2, 3)), "length 1 or 2")
})
