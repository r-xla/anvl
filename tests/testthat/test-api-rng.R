...
test_that("nv_sample_int", {
  ...
  values4 <- c(as_array(nv_sample_int(n = 6L, shape = 5000L, initial_state = state)[[2]]))
  ...
  expect_true(all(c(as_array(nv_sample_int(n = 1L, shape = 20L, initial_state = state)[[2]])) == 1L))
})
...
test_that("nv_sample from a population array", {
  ...
  many <- c(as_array(nv_sample(x = pop, shape = 500L, initial_state = state)[[2]]))
  expect_setequal(unique(many), c(10, 20, 30))

  # Unlike R's `sample()`, a length-one population is not a count
  expect_true(all(c(as_array(nv_sample(x = nv_array(6), shape = 5L, initial_state = state)[[2]])) == 6))
...
test_that("nv_sample and nv_sample_int compose inside jit", {
  ...
  expect_true(all(c(as_array(g(state, pop)[[2]])) %in% c(10, 20, 30)))
})
