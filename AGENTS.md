...
Primitives are `JitPrimitive` callables constructed by `new_primitive()` (defined in `R/primitive.R`). The returned object is both callable (it wraps `fn` with `jit()`) and carries an `AnvlPrimitive` metadata object via `attr(., "primitive")`. Primitives are stored as `prim_<name>` variables. `new_primitive()` evaluates its `fn` argument with `self` (the `AnvlPrimitive`) bound, so inside a primitive body you write `graph_desc_add(self, ...)` — never the primitive name as a string. A body written inside another function (the `make_*_op()` factories in `R/primitives.R`) takes `self` as an argument instead of closing over it, e.g. `new_primitive("add", make_binary_op(self, ...))`; a free `self` there would be a global variable that `R CMD check` flags. Interpretation rules are accessed via `prim_<name>[["<rule_type>"]]`:
