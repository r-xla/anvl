...
f64_sweep <- function(f, g) {
  ...
  set.seed(as.integer(paste0(setNames(1:26, letters)[c("a", "n", "v", "l")],
                             collapse = ""))) # low bits are random: seed for reproduciblity
  ...
  build <- function(hi, lo)
    readBin(writeBin(as.vector(if (LE) rbind(lo, hi) else rbind(hi, lo)),
                     raw(), size = 4L), "double", size = 8L, n = length(hi))
  ...
  bits64 <- function(x) {
    w <- matrix(readBin(writeBin(x, raw(), size = 8L), "integer",
                        size = 4L, n = 2 * length(x)), nrow = 2)
  ...
  inf_ranges <- with(inf_ranges, {
    side <- lo >= 2^31
    dbl <- function(p, w) { v <- build(as.integer(p - side * 2^31), w)
    ifelse(side, -v, v) }
    data.frame(bits_from = sprintf("0x%s00000000", hex32(lo)),
               bits_to   = sprintf("0x%sFFFFFFFF", hex32(hi)),
               x_from = dbl(lo, 0L), x_to = dbl(hi, -1L),
               blocks = hi - lo + 1)
  })
