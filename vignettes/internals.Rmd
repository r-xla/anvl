...
Therefore, `f_jit` also holds a cache -- pjrt's dispatcher, which keeps the compiled executables and picks one per call.
...
The body of the traced function runs only while tracing, so a counter inside it shows when the cache is used and when it is missed:

```{r}
traces <- 0L
counted <- function(x, y, op) {
  traces <<- traces + 1L
  f(x, y, op)
}
counted_jit <- jit(counted, static = 3L)

counted_jit(nv_scalar(1, "f32"), nv_scalar(2, "f32"), "add")
traces # 1: traced once
...
# a different static argument is a different program
counted_jit(nv_scalar(1, "f32"), nv_scalar(2, "f32"), "mul")
traces # 2
...
counted_jit(nv_array(c(1, 2), "f32"), nv_array(c(3, 4), "f32"), "add")
traces # 4
```
...
If `prim_convert()` were to follow the usual rule of committing its R inputs to their default data type, then `prim_convert(large_double, "i32")` would first convert the R `double` to the default float (`f32` as pjrt registers it -- see `default_dtypes()`) and then to an `i32`, which would result in a loss of precision.
...
when the staging widens past the data type the value would have committed to
