...
If `prim_convert()` were to follow the usual rule of materializing its R inputs at their default data type, then `prim_convert(large_double, "i32")` would first convert the R `double` to an `f32` (the default float data type on pjrt) and then to an `i32`, which would result in a loss of precision.
