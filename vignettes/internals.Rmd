...
If `prim_convert()` were to follow the usual rule of committing its R inputs to their default data type, then `prim_convert(large_double, "i32")` would first convert the R `double` to an `f32` (the default data type for floats) and then to an `i32`, which would result in a loss of precision.
