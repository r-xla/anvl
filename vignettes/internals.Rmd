...
2. A function without array inputs (a constructor) has no device to infer from.
   The constructor primitives (`prim_fill()`, `prim_iota()`) therefore `declare_device()` the device they were asked for into the trace, where it counts like the device of an array input.
