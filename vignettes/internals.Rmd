...
# same data types and shapes, different values and the same static argument
counted_jit(nv_scalar(-99, "f32"), nv_scalar(2, "f32"), "add")
traces # still 1: served from the cache
...
# a different data type is another one again
counted_jit(nv_scalar(1, "i32"), nv_scalar(2, "i32"), "add")
traces # 3
...
Because of its possibly problematic implications, a warning message is thrown
when the staging widens past the data type the value would have materialized at
anyway -- as above, where an R `double` reaches `i32` through `f64` under an
`f32` default. Under an `f64` default the staging brings in nothing new and
there is nothing to warn about; the same goes the other way, for a default
integer narrower than `i32`, where the staging is unavoidable and the caller
has no remedy to be pointed at.
