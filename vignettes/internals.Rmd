...
(the cache keys on R storage type and shape only), so it becomes a `GraphRData`
node, and which data type its input is supplied at is settled only once the body
has run. `finalize_rdata_inputs()` picks the narrowest data type that holds every
use site, records it on the input's own aval as an `RDataInput`, and adds a
