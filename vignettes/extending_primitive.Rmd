...
prim_repeat_along <- new_primitive(
  ...
  function(x, times, dim) {
    ...
    infer_fn <- function(x, times, dim) {
      if (!checkmate::test_integerish(dim, lower = 1, upper = naxes(x), len = 1L)) {
        cli::cli_abort("{.arg dim} must be between 1 and {naxes(x)}, but is {.val dim}")
      }
...
The idea here is the following, where we assume the input `x` has shape `(s_1, ..., s_n)`, which means that the output (and therefore it's gradient) has shape `(s_1, ..., s_{dim-1}, s_dim * times, s_{dim+1}, ..., s_n)`.
...
- `params`: Named list of the call's static parameters (here: `params$dim`, `params$times`)
