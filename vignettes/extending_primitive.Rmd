...
The inputs section of the printed graph shows both halves of it: the input's data type is the default the value committed to, and the `<- double` records that the caller supplies an R double rather than an array that already had one.
...
Committing each to its own default would make `prim_add(1, nv_scalar(2, "f64"))` an error -- the literal would become the default float and could meet an `f64` -- where it should give `f64`.
...
For the data type there is `peek_dtype()`, which reports the type the value *would* commit to; `dtype()` errors on an R value, because there is nothing to report until it is used.
