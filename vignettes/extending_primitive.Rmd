...
prim_repeat_along <- new_primitive(
  "repeat_along",
  function(operand, times, axis) {
    # type of operand is checked by graph_desc_add()
    infer_fn <- function(operand, times, axis) {
      if (!checkmate::test_integerish(axis, lower = 1, upper = naxes(operand), len = 1L)) {
        cli::cli_abort("{.arg axis} must be between 1 and {naxes(operand)}, but is {.val axis}")
      }
...
```{r}
prim_repeat_along[["stablehlo"]] <- function(operand, times, axis) {
  operands <- rep(list(operand), times)
  list(rlang::exec(stablehlo::hlo_concatenate, !!!operands, dimension = axis - 1L))
}
```
...
The idea here is the following, where we assume the input `operand` has shape `(s_1, ..., s_n)`, which means that the output (and therefore it's gradient) has shape `(s_1, ..., s_{axis-1}, s_axis * times, s_{axis+1}, ..., s_n)`.
...
The quickr rule is only required if you want the primitive to run under `local_backend("quickr")`; if you skip it, the primitive will still work on the xla backend.
