...
Note that in the `nv_*` wrapper function, the input may still be a bare R value -- one that has no data type yet (see `?RDataArray`).
`shape_abstract()` and `naxes_abstract()` answer for any arrayish input by first converting it to an `AbstractArray`.
