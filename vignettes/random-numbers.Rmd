...
# roll six dice
nv_sample_int(6L, state, 6L, replace = TRUE)[[2]]
```
...
nv_sample(8L, state, population, replace = TRUE)[[2]]
```

Both functions accept sampling weights via `prob`, which need not sum to one:

```{r}
nv_sample(8L, state, population, replace = TRUE, prob = nv_array(c(1, 1, 8)))[[2]]
```
