...
This article summarizes various important considerations in order to write efficient programs using {anvl}.
...
- [**Data types**](#data-types) -- working in single instead of double precision.
...
If you do want `f64` throughout, set it once rather than annotating every value: `options(anvl.default_dtypes = c(float = "f64"))`, or `with_default_dtypes(c(float = "f64"), ...)` for a single scope.
...
Value semantics and the fact that subset-assignment always copies were introduced in the [Next Steps article](next_steps.html#modifying-arrays).
...
One common scenario for this pattern is weight updates in training loops, where every step would otherwise allocate a new buffer for the parameters and leave the previous one for the garbage collector.
