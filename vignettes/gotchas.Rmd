...
The most important thing to know is that the operators `&`, `|` and `!` are *logical*, exactly like in base R -- a non-boolean operand is compared against zero and the result is boolean:
...
The similarly named functions `nv_and()`, `nv_or()`, `nv_xor()` and `nv_not()` are *bitwise* instead:
