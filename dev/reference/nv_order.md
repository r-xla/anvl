...
Returns the indices that would sort the array, like
[`base::order()`](https://rdrr.io/r/base/order.html): over every element
by default, or along one axis. It is the index twin of
...
``` r
nv_order(x, axis = NULL, decreasing = FALSE, stable = FALSE)
```
