...
- **Axis vs. dimension.** An *axis* is an index that identifies a
  direction of an array; a *dimension* is the *size* (extent) along an
  axis. For a `20x5x3` array the axes are `1`, `2`, `3` and the
  dimensions are `20`, `5`, `3`. Name identifiers accordingly: use
  `axis`/`axes` when the value is an index (or vector of indices), and
  `dim`/`dims` when it is a size (or shape vector). Helpers reflect
  this: `naxes(x)` is the number of axes (the rank), so
  `seq_len(naxes(x))` is the axis indices, and `shape(x)` / `dim(x)`
